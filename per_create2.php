...
if ( !empty($_POST)) { // if not first time through
	...
	if ($valid) 
	{
		...
		$sql = "INSERT INTO persons (fname,lname,email,phone,password,title,
		filename,filesize,filetype,filecontent) values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
		$q = $pdo->prepare($sql);
		$q->execute(array($fname,$lname,$email,$phone,$passwordhash,$title,
...
<body>
    <div class="container">
		...
		<div class="span10 offset1">
			...
			<form class="form-horizontal" action="per_create2.php" method="post" enctype="multipart/form-data">
