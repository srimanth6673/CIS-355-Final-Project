	...
	<body>
		<div class="container">
			...
			<div class="form-horizontal" >
				...
				<div class="row">
					<h4>Jobs Employee got hired</h4>
				</div>
				
				<?php
					$pdo = Database::connect();
					$sql = "SELECT * FROM applications, jobs WHERE appli_jobs_id = jobs.id AND appli_per_id = " . $id . " ORDER BY jobs_title ASC";
					$countrows = 0;
					foreach ($pdo->query($sql) as $row) {
						echo $row['jobs_title'] . ': ' .$row['jobs_requirements'] . ' - ' . $row['jobs_description'] . ' - ' . $row['jobs_payment'] . '<br />';
						$countrows++;
					}
