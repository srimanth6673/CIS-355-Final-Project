...
<body style="background-color: lightblue !important";>
    <div class="container">
		<img src="csis_banner.png" alt="banner">
		<div class="row">
			<h3>Jobs</h3>
		</div>
			...
			<table class="table table-striped table-bordered" style="background-color: lightgrey !important">
				...
				<tbody>
					<?php 
						...
						$sql = 'SELECT `jobs`.*, SUM(case when appli_per_id ='. $_SESSION['persons_id'] .' then 1 else 0 end) AS sumAssigns, COUNT(`applications`.appli_jobs_id) AS countAssigns FROM `jobs` LEFT OUTER JOIN `applications` ON (`jobs`.id=`applications`.appli_jobs_id) GROUP BY `jobs`.id ORDER BY `jobs`.jobs_title ASC';
						foreach ($pdo->query($sql) as $row) {
							echo '<tr>';
							echo '<td>'. $row['jobs_title'] . '</td>';
							echo '<td>'. $row['jobs_requirements'] . '</td>';
							echo '<td>'. $row['jobs_description'] . '</td>';
							//echo '<td>'. $row['jobs_payment'] . '</td>';
							if ($row['countAssigns']==0)
								echo '<td>'. $row['jobs_payment'] . ' - UNSTAFFED </td>';
							else
								echo '<td>'. $row['jobs_payment'] . ' (' . $row['countAssigns']. ' Employee)' . '</td>';
							echo '<td width=250>';
							echo '<td>';
							echo '<a class="btn" href="jobs_read.php?id='.$row['id'].'">Details</a> &nbsp;';
							if ($_SESSION['persons_title']=='Employee' )
								echo '<a class="btn btn-primary" href="jobs_read.php?id='.$row['id'].'">Employee</a> &nbsp;';
