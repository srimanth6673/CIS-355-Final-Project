...
if ( !empty($_POST)) { // if $_POST filled then process the form
	...
	if ($valid) { // if valid user input update the database
		...
		if($fileSize > 0) { // if file was updated, update all fields
			...
			$sql = "UPDATE persons  set fname = ?, lname = ?, email = ?, phone = ?, password = ?, title = ?, filename = ?,filesize = ?,filetype = ?,filecontent = ? WHERE id = ?";
			$q = $pdo->prepare($sql);
			$q->execute(array($fname, $lname, $email, $phone, $password, $title, $fileName,$fileSize,$fileType,$content, $id));
			Database::disconnect();
			...
			$pdo = Database::connect();
			$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
			$sql = "UPDATE persons  set fname = ?, lname = ?, email = ?, phone = ?, password = ?, title = ? WHERE id = ?";
			$q = $pdo->prepare($sql);
			$q->execute(array($fname, $lname, $email, $phone, $password, $title,  $id));
			Database::disconnect();
			header("Location: persons.php");
		}
	}
		...
		<div class="span10 offset1">
			...
			<form class="form-horizontal" action="per_update.php?id=<?php echo $id?>" method="post" enctype="multipart/form-data">
				...
				<div class="control-group <?php echo !empty($phoneError)?'error':'';?>">
					...
					<div class="controls">
						<input name="phone" type="text"  placeholder="Mobile Phone Number" value="<?php echo !empty($phone)?$phone:'';?>">
				...
				<div class="control-group">
					...
					<div class="controls">
						<select class="form-control" name="title">
							...
							if (0==strcmp($_SESSION['persons_title'],'Employee')) echo '<option selected value="Employee" >Employee</option>';
							else if($title==Employee) echo 
							'<option selected value="Employee" >Employee</option><option value="Employer" >Employer</option>';
							else echo
